...
export default function Footer({onChangePage,page,totalPages}) {
  ...
  return (
    <div className="flex flex-row justify-between items-center py-2 px-4 border-t">
      <ButtonPagination nextPage={-1} disabled={page===1} arrowIcon={arrowBack} onChangePage={onChangePage} classButton={buttonBack} text={"Previous"} orderIcon={1}/>
      <div>
        <ul>
          <li>
      ...
      <ButtonPagination nextPage={1} disabled={page === totalPages} arrowIcon={arrowNext} onChangePage={onChangePage} classButton={buttonNext} text={"Next"} orderIcon={2}/>
    </div>
  );
}
